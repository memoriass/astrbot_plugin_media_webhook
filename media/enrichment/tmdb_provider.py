...
class TMDBProvider(MediaEnrichmentProvider, MediaImageProvider, BaseProvider):
    ...
    async def get_image(self, media_data: dict) -> str:
        ...
        try:
            ...
            if item_type == "Episode" and season_number and episode_number:
                ...
                if tmdb_id:
                    details = await self._get_tmdb_episode_details(tmdb_id, season_number, episode_number)
    ...
    async def _enrich_movie_by_id(self, media_data: dict, movie_id: str) -> dict:
        ...
        data = await self._http_get(url, params={"api_key": self.tmdb_api_key, "language": "zh-CN"})
        if data:
            media_data.update({
                "tmdb_id": data.get("id"),
    ...
    async def _enrich_tv_by_id(self, media_data: dict, tv_id: str) -> dict:
        ...
        if data:
            ...
            if season and episode:
                ep_data = await self._get_tmdb_episode_details(data.get("id"), season, episode)
                if ep_data:
                    media_data.update({
                        "item_name": ep_data.get("name") or media_data.get("item_name"),
                        "overview": ep_data.get("overview") or media_data.get("overview"),
                        "tmdb_enriched": True
                    })
        return media_data
    ...
    def _find_best_match(self, query: str, results: list, key: str) -> Optional[dict]:
        """寻找最佳匹配（简单的名称清理和包含检查）"""
        ...
        for res in results:
            ...
            # 1. 完全一致
            if query_clean == res_clean: return res
            # 2. 包含关系
            if query_clean in res_clean or res_clean in query_clean: return res
    ...
    async def _find_tmdb_id_by_external(self, external_id: str, source: str) -> Optional[str]:
    ...
    async def _get_tmdb_episode_details(self, tv_id: Any, season: Any, episode: Any) -> Optional[dict]:
