...
class MediaWebhookPlugin(Star):
    ...
    async def handle_webhook(self, request: Request) -> Response:
        ...
        try:
            ...
            # 添加到消息队列
            await self._add_to_queue(media_data, source)
    ...
    async def _add_to_queue(self, media_data: Dict, source: str):
        ...
        logger.info(f"新 {item_type} 通知已加入缓存队列 [来源: {source_name}] {'(含图片)' if image_url else '(无图片)'}")
        logger.info(f"当前队列状态: {queue_size} 条消息，阈值: {self.batch_min_size}")

        # 记录首次加入队列的时间（如果队列之前为空）
        if queue_size == 1:
            self.last_batch_time = time.time()
            logger.info(f"开始新的缓存周期，将在 {self.batch_interval_seconds} 秒后发送")
    ...
    def _determine_send_strategy(self, protocol_info: Dict, queue_size: int) -> tuple:
        """确定发送策略 - 基于缓存时间和平台能力"""
        # 只有超过缓存时间才发送
        if not self._is_cache_timeout():
            return False, "cache_wait"

        # 根据队列大小和平台能力选择发送方式
        if queue_size > self.batch_min_size and protocol_info["supports_forward"]:
            return True, "batch_forward"  # 超过阈值且支持合并转发
        else:
            return True, "individual_send"  # 不超过阈值或不支持合并转发
    ...
    async def _execute_send_strategy(self, strategy: str, protocol_info: Dict):
        ...
        try:
            ...
            elif strategy == "individual_send":
                logger.info(f"使用单独发送 {len(messages)} 条消息")
                await self._send_individual_burst(messages, protocol_info)
            else:
                logger.warning(f"未知发送策略: {strategy}，使用单独发送")
                await self._send_individual_burst(messages, protocol_info)
        ...
        while True:
            try:
                ...
                if self.message_queue and self._is_cache_timeout():
                    queue_size = len(self.message_queue)
                    logger.info(f"缓存超时触发：发送 {queue_size} 条缓存消息")
                    ...
                    # 根据队列大小和平台能力选择发送策略
                    if queue_size > self.batch_min_size and protocol_info["supports_forward"]:
                        strategy = "batch_forward"
                    else:
                        strategy = "individual_send"

                    # 执行发送
                    await self._execute_send_strategy(strategy, protocol_info)
